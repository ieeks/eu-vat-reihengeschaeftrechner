...
- [x] **Mode 2 Drop-Shipment für Drittland-Kunde mit EU-Warenempfänger** — Fall EPROHA(AT) → CH-Kunde → SK: Ware bleibt in der EU → ig. Reihengeschäft statt Schweiz-Export. Flag `euGoodsRecipient` deaktiviert die CH/GB/isNonEU-Export-Branches; neuer Sub-Branch `bIsNonEU` (ig. Lieferung AF 0% nur mit EU-UID, Dreieck gesperrt, Kunde muss sich im Bestimmungsland registrieren); `renderContextToggles()` zeigt Drop-Shipment auch bei Drittland-Kunde. edge-cases F4. **Browserabnahme offen.**
