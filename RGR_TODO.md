...
- [ ] **Meldefristen je Land** (aus `EPDE_Steuerbuch.xlsx`, Sheet „UID-Nr.": DE 10., IT 16., BE/EE 20., LV 23., LT/CZ/PL 25., SI/NL letzter Werktag) optional in `buildMeldepflichten()` einpflegen
...
- [x] **Mode 2 Drop-Shipment für Drittland-Kunde mit EU-Warenempfänger** — Fall EPROHA(AT) → CH-Kunde → SK: Ware bleibt in der EU → ig. Reihengeschäft statt Schweiz-Export. Flag `euGoodsRecipient` deaktiviert die CH/GB/isNonEU-Export-Branches; neuer Sub-Branch `bIsNonEU`; `renderContextToggles()` zeigt Drop-Shipment auch bei Drittland-Kunde. edge-cases F4. **Browserabnahme abgenommen (Session 28).**
- [x] **Mode 2 UID-Picker für Drittland-Kunde** — State `mode2CustUid` + `setMode2CustUid()`, Picker im Drop-Shipment-Panel. `bIsNonEU`-Branch klassifiziert 4 Fälle: keine UID→20% AT (A2) · Abgangsland-UID(AT)→20% AT · Bestimmungsland-UID→ig. Lieferung AF 0% + ig.Erwerb/Inlandslieferung · Dritt-MS-UID→Dreiecksgeschäft Art.141. `isTriangle=triByThirdUid`. edge-cases F4 UID-Tabelle. **Browserabnahme abgenommen (Session 28).**
...
- [x] **QuickCheck Lohnveredelung** — umgesetzt (Session 28). Gemeinsame reine Logik `computeLohn()` aus `analyzeLohn()` extrahiert (EINZIGE Quelle); `analyzeLohn()` leitet Fallauswahl daraus ab (HTML unverändert, OT-M5 grün); QuickCheck-`lohn`-Modus rendert 3-Schritt-Spiegel (Einkauf/Veredelung-RC/Verkauf) + Reg-Banner + 2 Schalter (Warenweg direkt/über-dich, Ware-kommt-zurück lit. f) aus `computeLohn()`. Coming-Soon entfernt. Tests LV-01…05 (43 Output-Tests grün).
...
- [x] **Einführer-Toggle + SAP-MWSKZ für CH/GB (3P)** — Toggle in `analyzeCH`/`buildCHExportResult`/`analyzeGBImport`/`buildGBExportResult` (statische Karten ersetzt); `_importerConsequence` länderbewusst (CH/GB-Spezifika) + SAP-Codes je `importerRole` via `sapBadgeBoth` (nur vorhandene Codes, EUSt-/Lückenhinweis). 33 Tests grün (Session 34). Offen: optional `SAP_TAX_MAP` um EUSt-/fehlende Länder-MWSKZ ergänzen (sobald Excel-Codes vorliegen); 4P-Drittland-Spezialpfad; pausierter app.js-Split (Tier 2).
- [x] **2P-CH/GB-Export-Karten auf Einführer-Toggle umgestellt** — `analyze2()`-Zweige `dest==='CH' && !euGoodsRecipient` und `dest==='GB' && !euGoodsRecipient`: statisches DAP/EXW-vs-DDP-Zwei-Karten-Grid → `_importerToggle(country, 'export')`. Konsistent mit 3P-Drittland-Pfaden und generischem 2P-Pfad (TR/RS/BA/RU). Umgebende Hinweise/Konsi-Lager/SAP-Zeilen unverändert. JSDOM-verifiziert, `npm run check` grün (Session 35).
...
- [x] **GB/CH-Export: Eingangsrechnung-MWSKZ + Leg-Zuordnung bei `transport=supplier`** — gefixt (Session 28). Repro: `?co=EPDE&mode=3&transport=supplier&countries=DE,DE,GB&mePos=2`. Zwei Befunde behoben:
  1. **Eingangsrechnung-Zeile ergänzt:** `_importerConsequence(country, 'export', movingL1)` zeigt im `customer`-Zweig jetzt beide Zeilen. `movingL1=true` → Eingang 0 % (bewegte Ausfuhr, kein Vorsteuerabzug), Ausgang L2 = Drittland-Inland (`sc(country,'domestic','seller')`, EPDE-GB ohne Code → ⚠). `movingL1=false` → Eingang L1 ruhend **VD**, Ausgang L2 **G0**. 2P-Aufruf (`movingL1` undefined) bleibt unverändert.
  2. **`isIExporter` parteibasiert** (`!movingL1`) in `buildGBExportResult`/`buildCHExportResult`/`buildThirdExportResult` — Länder-Vergleich entfernt (schlug bei s1=s2=DE fälschlich an). Tote `*ExportBanner`-Funktionen nicht angefasst.
  → Verifiziert: OUTPUT_TESTS `OT-GBX-01` (supplier → Vorlieferant=Exporteur, Eingangsrechnung da, kein „Du bist Exporteur") + `OT-GBX-02` (customer → EPDE=Exporteur, L1=VD). CH-Gegenprobe grün. 38 Output-Tests grün.
