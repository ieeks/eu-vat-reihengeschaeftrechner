...
| `vat-knowledge/` | 18 Markdown-Dateien + Index — EU/AT/DE/CH/NL Steuerrecht ↔ Code-Mapping |
...
docs/assets/scripts/app.js
  ...
  _exwExportHint(country) ← Hinweis-Renderer (KEINE Steuerlogik): EXW mit Drittlandskunde ist zollrechtlich problematisch — der Ausführer der EU-Ausfuhranmeldung muss EU-ansässig sein (Art. 1 Nr. 19 UZK-DA i.d.F. VO (EU) 2018/1063), ein Drittlandskunde bräuchte einen indirekten Vertreter. Empfehlung FCA statt EXW (Ausführerstellung + EORI + Ausgangsvermerk/MRN bleiben bei uns; Nachweis § 7 Abs. 3 UStG AT bzw. § 6 Abs. 4 UStG i.V.m. §§ 9 ff. UStDV). Trigger am Call-Site: Kunde holt ab UND Ziel ist Drittland — 2P analyze2 (CH/LI/GB/generisch via isAbholung), 3P buildCHExportResult/buildGBExportResult/buildThirdExportResult via getCanonicalTransport()==='customer'
  ...
  computeLohn({company,sup,con,cus,lvDirect,litF,homeHandover}) ← REINE gemeinsame Steuerlogik Lohnveredelung (Mode 5 + QuickCheck). Liefert {inland, supIsHome, sameConCus, litF, homeHandover, regRisks[], steps:[{key:einkauf|veredelung|verkauf, kind, title, taxInfo, sap, note, regRisk}]}. EINZIGE Quelle der Fallunterscheidung — kein DOM. **Schritt 2 (Veredelungsleistung) kommt aus `veredelungStep()` und hängt allein an `conIsHome` (con===myHome), NICHT an sup===con:** Leistungsort ist immer Art. 44 = mein Sitz, aber Reverse Charge setzt einen im Ausland ansässigen Leistenden voraus (Art. 196 MwStSystRL / § 13b Abs. 2 Nr. 1 UStG / § 19 Abs. 1 UStG AT). con===myHome → kind `inland-service`, rc:false, lokale MwSt (EPDE/DE=VD); sonst kind `rc` (EPROHA/AT=RC). Gilt auch im inland-Zweig: EPROHA (Sitz AT) mit sup=con=DE hat Inlandseinkauf, aber RC auf die Werkleistung. Zweige: (a) inland (sup===con) rein innerstaatlicher Wareneinkauf; (b) supIsHome (sup===myHome, con≠home): Inlandskauf + EU-Veredelung — homeHandover=true → Inlandslieferung Regelsatz (SAP domestic, AT=V2) + Verbringen Heimat↔con durch Art. 17 Abs. 2 lit. f gedeckt (keine con-Reg); homeHandover=false → ig. Lieferung Heimat→con (Verfügungsmacht im con-Land, z.B. DAP) + ig. Erwerb con (SAP via con-UID, DE=VH) → con-Reg nötig, lit. f schützt Hinlieferung NICHT; (c) Normalpfad sup≠con≠home (lvDirect). Verfügungsmacht-Schalter (lohnHome / window.lvHomeHandover, QC qcState.lohnHome) nur sichtbar wenn Lieferant im Heimatland (syncLohnHomeVisibility). Eigener Render-Zweig in analyzeLohn (if L.supIsHome) mit Lesart-Banner + DAP-Warnung (Lesart-Banner + Reg-Hinweise sind litF-abhängig — NICHT nur homeHandover). ig-sale-Schritte (Ware bleibt im Veredelungsland) nutzen `sapFrom(c,t,role)` = getSapCode(...,uidCountry=c): das Kennzeichen hängt am UID-Land des ABGANGSLANDS (IG-Lieferung ab DE = DH, ab PL/EPDE = T1), nicht an der Heimat-UID — ohne den Hint fällt _sapEffectiveCountry auf home zurück (EPROHA→AT→AF, falsch). Kein Eintrag für dieses UID-Land → kein Code (nie ein erfundener). Dazu igSaleNote() (Lieferort=Abgangsland, Art. 32, Meldung über dessen UID) und verbringenSapHint() (meldepflichtiges ig. Verbringen: Ausgang AF / Eingang VH).
  computeLohn → Normtext + Code-Mapping: vat-knowledge/eu/art17_verbringen.md (Pflichtlektüre). ART17_ABS3_HINT (Zeitpunktregel Art. 17 Abs. 3) wird NUR bei verbringen.reason==='no-return' ausgegeben — bei 'not-dispatched' wurde Abs. 2 nie in Anspruch genommen. lit.-e-Auslegung siehe rechtskonformitaet.md § D3 (bewusst konservativ)
  computeLohn → STEP_META hängt je Schritt uidCountry/sapTreatment/role an (abgeleitet aus kind, keine neue Entscheidung); `verbringen` = {from,to,meldepflichtig}|null macht das Eigen-Verbringen explizit; alle Zweige returnen über finish()
  renderLohnBegruendung() / renderLohnInvoice() / renderLohnMelde() ← Modus-5-Experten-Tabs aus `_lohnLast` (von analyzeLohn gesetzt). ERSETZEN im Mode-5-Zweig renderExpertBegrundung/-Invoice/-Melde/-RPA: die 3P-Engine kennt weder Werkleistung noch Verbringen und widersprach dem Ergebnis-Tab (Reihengeschäfts-Prosa, „Keine ZM-Pflicht" trotz ig. Lieferung ab con, Musterrechnung ans falsche Land). renderExpertLegal() behält seinen eigenen Mode-5-Zweig und hängt an tab-begrundung an. KEINE Steuerlogik in den Renderern. Regressionslauf: scripts/test-lohn-tabs.mjs (15 Konstellationen, in npm test verkettet)
...
**Einführer-Toggle in ALLEN 3P-Drittland-Pfaden:** `_importerToggle(country, direction)` wird jetzt auch in `analyzeCH` (Import-Case 2), `buildCHExportResult`, `analyzeGBImport`, `buildGBExportResult` gerendert (statische DAP/DDP-Karten ersetzt). `_importerConsequence()` ist länderbewusst (CH: BAZG/8,1 %/Art. 67+28 MWSTG · GB: HMRC/UK VAT) und blendet je `importerRole` real existierende SAP-MWSKZ via `sapBadgeBoth` ein (Export A0/G0/D0 bei customer · Anschlusslieferung z.B. BE BS/BI, CH B5 bei self); fehlt ein Code bzw. für die EUSt → „kein AP-MWSKZ"-Hinweis (keine erfundenen Codes). **Im 2P-Modus (`analyze2`) nutzen CH und GB den eigenen Incoterm-Umschalter „Konzept 02"** (`buildMode2IncoExport(country)`, Zweige `dest==='CH' && !euGoodsRecipient`, `dest==='GB' && !euGoodsRecipient`) — NICHT den `_importerToggle`. Ein dezenter Segmented-Switch **DAP/EXW ↔ DDP** (Default `dap`, State `mode2Incoterm`, Setter `setMode2Incoterm()`→`renderResult()`, in getState/loadState persistiert) blendet je EINE Detailkarte ein: bewegte Rechnung-an-Kunde (DAP/EXW 0 % A0 · DDP CH 8,1 % B5 / GB 20 % „kein AP-MWSKZ"), „Wer verzollt", Pflichten, plus die fixe Zusicherung „immer nur eine Kundenrechnung". Der **Eigenimport** (Ausfuhr eigener Ware A0 → Inlandverkauf) steht bewusst nur als **dezente Fußnote** im DDP-Fall (`.m2i-foot`), nicht als gleichwertiger Beleg-Block. CSS-Klassen-Präfix `m2i-` (eigener Block in `app.css`, kollisionsfrei, Tokens aus `:root`). Hervorgegangen aus einem „Claude design"-Vergleich dreier Konzepte; Karten/Toggle-Vorstufen wurden verworfen. **Konsignationslager-CH-Block (`buildKonsiLagerCH`) nur bei `mode2Incoterm==='ddp'` prominent** (gehört in die DDP-Welt: EPROHA = Einführer + CH-Registrierung); bei DAP/EXW nur ein dezenter Verweis. **Auch die generischen Drittländer TR/RS/BA nutzen `buildMode2IncoExport(dest)`** (Konzept 02) — DAP/EXW als Normalfall (0 % A0); der **DDP-Fall ist dort eine ehrliche Warnkarte `.m2i-card.warn` „So nicht möglich"** (keine EPROHA-Registrierung → Hinweis „bräuchte Steuerregistrierung/Fiskalvertreter, bitte DAP/EXW wählen"), KEINE Schein-Detailkarte; Config-Fallback in `buildMode2IncoExport` (`ddpWarn:true`, Satz aus `rate(country)`). **Nur RU behält den `_importerToggle`** via `_importerToggle('RU', 'export', undefined, /*twoParty*/ true)` — dort steht die Sanktionswarnung (`_thirdCountryNote`) im Vordergrund, eine DAP/DDP-Karte wäre fast irreführend. Der 4. Param **`twoParty=true`** zeigt nur `Wir (DDP)` / `Kunde (DAP/EXW)` — die `supplier`/„Lieferant (DDP)"-Option entfällt (eigene Ware ab Lager, kein Vorlieferant). Stale `importerRole==='supplier'` aus einem 3P-Kontext wird im 2P auf `self` geklemmt; `_importerConsequence(..., roleArg)` nimmt die effektive Rolle als optionalen Override. `_thirdCountryNote(dest,'export')` (Sanktionen/Zollunion/SAA) bleibt in ALLEN generischen Pfaden prominent oben.

**Incoterms sind KEIN eigener State (bewusst).** Umsatzsteuerlich wirken sie nicht selbst — sie sind nur Indiz für Transportveranlasser (Art. 36a), Verfügungsmacht/Lieferort und Einführer, und alle drei fragt das Tool bereits einzeln ab (Transport-Feld · `lohnHome`/`homeHandover` · `importerRole`). Ein Incoterm-Picker daneben wäre redundant und könnte dem Transport-Feld widersprechen → nicht einführen (Regel 11). Einzige Incoterm-Eingabe bleibt `mode2Incoterm` im 2P-Drittland-Export, und dort ist die modellierte Dimension ebenfalls „wer verzollt", nicht der Incoterm.

**Toggle-Labels = Rolle, Incoterm nur Zusatz:** `_importerToggle` zeigt **Wir · DDP** / **Kunde · EXW/FCA/DAP** / **Lieferant · DDP** — NICHT mehr das Sammel-Label „Kunde (DAP/EXW)". Grund: EXW und DAP führen zwar beide zum Kunden als Einführer, sind für die **Transportzuordnung** aber Gegensätze (EXW = Kunde holt ab, DAP = wir liefern zu); in der Kette konnte so „Kunde (DAP/EXW)" neben Transport „A — Lieferant" stehen, ohne dass der Widerspruch sichtbar wurde. Darunter eine **Indiz-Note nur bei `!twoParty`** (3P/4P): Incoterms sind hier nur Indiz für den Einführer, die Transportzuordnung (Art. 36a) kommt aus dem Transport-Feld. Im 2P wäre der Hinweis Rauschen (nur eine Lieferung). Prosa in `_importerConsequence`/`drittlandReg*` durchgängig `(EXW/FCA/DAP)`.

**SAP-Divergenz EXW (offen, siehe RGR_TODO):** Die SAP-Findung modelliert EXW als eigenes Feld und dreht die UID aufs **Lieferantenland** (`vat-knowledge/plants_abroad/README.md` Punkt 5; fehlt dort eine eigene UID, greift der Ship-to-Fallback vor dem Buchungskreis). `buildSapInputHint`/`SAP_CUSTTAX_MAP` kennen diese Dimension nicht → für denselben Fall können Tool und SAP abweichende MWSKZ ergeben. Erst mit dem SAP-Team klären, dann höchstens als SAP-Eingabe-Hinweis abbilden — nie als Steuerlogik.

**Liechtenstein (LI) = Schweizer MWST-Raum:** LI bildet mit CH einen gemeinsamen MWST-Raum (Zollvertrag 1923; Schweizer MWSTG gilt in LI); EPROHAs CH-Registrierung (`CHE-113.857.016 MWST`) deckt LI mit ab. `LI` ist im `EU`-Array (`nonEU:true, std:8.1, swissVatArea:true`, Flag 🇱🇮) → als Zielland wählbar. **`isCH(c)` erfasst `CH` UND `LI`** → LI läuft durch ALLE CH-Pfade (`analyzeCH`/`buildCHExportResult`/`analyzeCHInland`, Mode-3-Dispatch `hasCH`). `SAP_TAX_MAP.EPROHA.LI` (Ausfuhr A0 · domestic B5/IB) und `SAP_TAX_MAP.EPDE.LI` (Ausfuhr G0) spiegeln die CH-Einträge. Mode 2 hat einen **eigenen additiven `dest==='LI'`-Branch** (CH-Branch unangetastet) mit `buildMode2IncoExport('LI')` (DAP/EXW→A0 · DDP→B5 8,1 %, Registrierung via CH-UID) + Konsi-Lager-Hinweis. `drittlandRegCountry` behandelt LI-Reg als über CH-UID abgedeckt (keine falsche Registrierungswarnung). KEINE neue Steuerlogik — reine CH-Spiegelung.
