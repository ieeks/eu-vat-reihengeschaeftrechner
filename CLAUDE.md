...
**Einführer-Toggle in ALLEN 3P-Drittland-Pfaden:** `_importerToggle(country, direction)` wird jetzt auch in `analyzeCH` (Import-Case 2), `buildCHExportResult`, `analyzeGBImport`, `buildGBExportResult` gerendert (statische DAP/DDP-Karten ersetzt). `_importerConsequence()` ist länderbewusst (CH: BAZG/8,1 %/Art. 67+28 MWSTG · GB: HMRC/UK VAT) und blendet je `importerRole` real existierende SAP-MWSKZ via `sapBadgeBoth` ein (Export A0/G0/D0 bei customer · Anschlusslieferung z.B. BE BS/BI, CH B5 bei self); fehlt ein Code bzw. für die EUSt → „kein AP-MWSKZ"-Hinweis (keine erfundenen Codes). **Im 2P-Modus (`analyze2`) bleiben CH und GB bewusst bei den statischen DAP/EXW-vs-DDP-Karten** (`dest==='CH' && !euGoodsRecipient`, `dest==='GB' && !euGoodsRecipient`) — der Toggle wurde dort testweise eingebaut, aber wieder zurückgebaut, weil die Karten den 2P-Eigenware-Fall klarer zeigen (zwei Incoterm-Varianten nebeneinander, ohne die A0/B5-SAP-Codes, die im DDP-Fall den Zwei-Belege-Aspekt — Eigenimport A0 ↔ Kundenrechnung B5 — verwirrend wirken ließen). **Nur der generische 2P-Drittland-Pfad (TR/RS/BA/RU) nutzt den Toggle** via `_importerToggle(dest, 'export', undefined, /*twoParty*/ true)`. Der 4. Param **`twoParty=true`** zeigt nur `Wir (DDP)` / `Kunde (DAP/EXW)` — die `supplier`/„Lieferant (DDP)"-Option entfällt (eigene Ware ab Lager, kein Vorlieferant in der Kette). Stale `importerRole==='supplier'` aus einem 3P-Kontext wird im 2P auf `self` geklemmt; `_importerConsequence(..., roleArg)` nimmt die effektive Rolle als optionalen Override. **Offen/Designfrage:** ob 2P CH/GB perspektivisch auf den Toggle (mit klarer Beleg-1-/Beleg-2-Trennung) umgestellt werden — bis dahin Karten.
