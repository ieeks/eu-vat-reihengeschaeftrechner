...
const uidFindung = [
  ...
  { ebene: 'B-MWSKZ-Findung', prio: 1, regel: 'B1', bedingung: "vatTreatment ∈ {ic-exempt, ic-acquisition, dreiecks, export}",
    ...
    hinweis: 'Beispiel: IG-Erwerb in AT mit AT-UID = VE, in DE mit DE-UID = VH. Eigene bewegte Ausgangslieferung: siehe B1a.' },
  { ebene: 'B-MWSKZ-Findung', prio: 1, regel: 'B1a', bedingung: 'eigene BEWEGTE Ausgangslieferung (ic-exempt / export, kein Dreieck)',
    ergebnis: 'MWSKZ aus der UID des ABGANGSLANDS; uidOverride (dem Vorlieferanten mitgeteilte UID) und companyHome spielen keine Rolle. Keine UID/kein Eintrag im Abgangsland → KEIN Code + Registrierung', uid_land: 'dep',
    rechtsgrundlage: 'Art. 32 + 138 MwStSystRL · § 14a Abs. 3 / § 18a UStG — Lieferort und Meldung im Abgangsland', quelle: 'buildKurzbeschreibung ownSupplyNotes() · buildDeliveryBox() · buildVergleichTab()',
    hinweis: 'EPROHA ab DE = DH (nicht AF, auch wenn die AT-UID dem Lieferanten mitgeteilt wurde). EPDE ab IT ohne IT-UID = KEIN (Matrix-Zeile 38), nicht DH.' },
